...
# Build and install your project (hyvec in this case)
RUN cmake -G Ninja -B /project/build -S /project
RUN cd /project/build
RUN ninja -C build
RUN ninja -C build install

# Deploy the build results to the /deploy directory
RUN mkdir -p /deploy
RUN cp -r /usr/local/bin/hyvec /deploy/hyvec
