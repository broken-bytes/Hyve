#pragma once

namespace Hyve::Lexer::Keywords {
    constexpr const char* KEYWORD_ACTOR = "actor";
    constexpr const char* KEYWORD_ALLOC = "alloc";
    constexpr const char* KEYWORD_AS = "as";
    constexpr const char* KEYWORD_ASYNC = "async";
    constexpr const char* KEYWORD_AWAIT = "await";
    constexpr const char* KEYWORD_BREAK = "break";
    constexpr const char* KEYWORD_CAPTURED = "captured";
    constexpr const char* KEYWORD_CATCH = "catch";
    constexpr const char* KEYWORD_CONTINUE = "continue";
    constexpr const char* KEYWORD_CONTRACT = "contract";
    constexpr const char* KEYWORD_DEALLOC = "dealloc";
    constexpr const char* KEYWORD_DEFER = "defer";
    constexpr const char* KEYWORD_DO = "do";
    constexpr const char* KEYWORD_ELSE = "else";
    constexpr const char* KEYWORD_ENUM = "enum";
    constexpr const char* KEYWORD_FALSE = "false";
    constexpr const char* KEYWORD_FILEPRIVATE = "fileprivate";
    constexpr const char* KEYWORD_FOR = "for";
    constexpr const char* KEYWORD_FROM = "from";
    constexpr const char* KEYWORD_FUNC = "fn";
    constexpr const char* KEYWORD_IF = "if";
    constexpr const char* KEYWORD_IMPORT = "use";
    constexpr const char* KEYWORD_INIT = "init";
    constexpr const char* KEYWORD_INTERNAL = "internal";
    constexpr const char* KEYWORD_LET = "let";
    constexpr const char* KEYWORD_MODULE = "module";
    constexpr const char* KEYWORD_NULL = "null";
    constexpr const char* KEYWORD_OPEN = "open";
    constexpr const char* KEYWORD_PRIVATE = "private";
    constexpr const char* KEYWORD_PUBLIC = "public";
    constexpr const char* KEYWORD_RETURN = "return";
    constexpr const char* KEYWORD_SELF = "self";
    constexpr const char* KEYWORD_STRUCT = "struct";
    constexpr const char* KEYWORD_TASK = "task";
    constexpr const char* KEYWORD_THROW = "throw";
    constexpr const char* KEYWORD_THROWS = "throws";
    constexpr const char* KEYWORD_TRUE = "true";
    constexpr const char* KEYWORD_TRY = "try";
    constexpr const char* KEYWORD_UNTIL = "until";
    constexpr const char* KEYWORD_VAR = "var";
    constexpr const char* KEYWORD_WHEN = "when";
    constexpr const char* KEYWORD_WHILE = "while";
    constexpr const char* KEYWORD_WITH = "with";
    constexpr const char* KEYWORD_YIELD = "yield";
}